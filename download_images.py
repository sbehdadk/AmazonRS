...
def get_the_URL():
    adress = []
    i = 0
    for metadata in parse("./dataset/metadata.json.gz"):
            ...
            if not os.path.isfile("/media/sina/Daten/AmazonRS/dataset/amazon/"+ namestring):
                if "imUrl" in metadata:
                    ...
                    if "categories" in metadata:
                        ...
                        #print(filename)
                        try:
                            request.urlretrieve(metadata["imUrl"],"/media/sina/Daten/AmazonRS/dataset/amazon/"+ filename)
                        except:
                            pass

                        adress.append(metadata['imUrl'])
                    else:
                        filename = "unknown" + "_" + str(metadata["asin"])
                        request.urlretrieve(metadata["imUrl"],"/media/sina/Daten/AmazonRS/dataset/amazon/"+ filename)
                        continue
